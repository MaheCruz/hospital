...
function getCitasDetalle($periodo) {
    global $con;  // Asegúrate de que esta variable contenga tu conexión a la base de datos
    $sql = "";
    $today = date('Y-m-d');
    $startOfWeek = date('Y-m-d', strtotime('Monday this week'));
    $endOfWeek = date('Y-m-d', strtotime('Sunday this week'));  // Asegúrate de que el fin de semana sea el domingo
    ...
    if ($periodo == 'diario') {
        $sql = "SELECT doctors.doctorName, users.fullName as patientName, appointment.appointmentTime, appointment.appointmentDate, doctors.consultorio
                ...
                WHERE DATE(appointment.appointmentDate) = '$today'";
    } else if ($periodo == 'semanal') {
        $sql = "SELECT doctors.doctorName, users.fullName as patientName, appointment.appointmentTime, appointment.appointmentDate, doctors.consultorio
                FROM appointment
                JOIN doctors ON doctors.id = appointment.doctorId
                JOIN users ON users.id = appointment.userId
                WHERE DATE(appointment.appointmentDate) BETWEEN '$startOfWeek' AND '$endOfWeek'";
    }

    $result = $con->query($sql);
    $citas = [];
    while ($result && $row = $result->fetch_assoc()) {
        $citas[] = $row;  // Verifica que cada fila se añade correctamente al array
    }
    return $citas;
}
