...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    $searchName = '%' . $_POST['searchName'] . '%';  // Tomamos la entrada y agregamos comodines
    ...
    if ($especialidad === 'all') {
        $sql = "SELECT d.doctorName AS docname, u.fullName AS pname, a.*, d.consultorio 
                ...
                WHERE DATE(a.appointmentDate) BETWEEN ? AND ? 
                  AND (d.doctorName LIKE ? OR u.fullName LIKE ?)
    ...
    if ($especialidad === 'all') {
        $stmt->bind_param("ssss", $fromdate, $todate, $searchName, $searchName);
    } else {
        $stmt->bind_param("sssss", $fromdate, $todate, $especialidad, $searchName, $searchName);
    }
    ...
    if (!$result || $result->num_rows == 0) {
        echo "<script>alert('No se encontraron consultas para los criterios seleccionados.');</script>";
    }
}
?>
